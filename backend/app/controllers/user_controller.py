...
from auth import decode_access_token, create_access_token, COOKIE_NAME, COOKIE_SECURE, COOKIE_SAMESITE, ACCESS_TOKEN_EXPIRE_MINUTES
...
from dependencies import get_current_user, require_admin, require_analyste
...
@router.post("/login")
async def login(form_data: LoginSchema, response: Response, request: Request, background_tasks: BackgroundTasks):
    """
    Étape 1 de l'authentification (Bank Grade) : Email + Mot de passe.
    Si OK, déclenche le MFA (OTP par email).
    """
    user = await service.login(form_data.email, form_data.password)
    
    # 🔐 Déclenchement du MFA (OTP par email)
    # Dans un système bancaire, on ne donne jamais le token directement après le mot de passe.
    await service.request_mfa_otp(user["email"])

    return {
        "message": "Authentification (Step 1) réussie. Un code OTP a été envoyé à votre adresse email.",
        "mfa_required": True,
        "email": user["email"],
...
async def verify_otp(data: dict, response: Response, request: Request, background_tasks: BackgroundTasks):
    """
    Étape 2 de l'authentification : Vérification du code OTP (ou succès reco faciale fallback).
    """
    ...
    user = await service.verify_mfa_otp(email, code)
    ...
    return {
        "message": "Connexion réussie (MFA validé)",
        "role": user["role"],
    }

@router.post("/logout")
async def logout(response: Response, request: Request, background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user)):
...
async def update_user(user_id: str, data: dict, request: Request, background_tasks: BackgroundTasks, admin=Depends(require_admin)):
    # 🔍 Capturer l'état AVANT
    user_before = await service.get_user_by_id(user_id)
...
async def delete_user(user_id: str, request: Request, background_tasks: BackgroundTasks, admin=Depends(require_admin)):
    # 🔍 Capturer l'état AVANT
    user_before = await service.get_user_by_id(user_id)
...
async def firewall_upload(file: UploadFile = File(...),
                          current_user: dict = Depends(require_analyste)):
...
async def firewall_reset(current_user: dict = Depends(require_analyste)):
...
async def database_run(background_tasks: BackgroundTasks,
                 current_user: dict = Depends(require_analyste)):
...
async def db_reset(current_user: dict = Depends(require_analyste)):
...
@router.post("/os/run", status_code=status.HTTP_202_ACCEPTED, tags=["Pipeline OS"])
async def os_run(background_tasks: BackgroundTasks,
    ...
    return {"message": "Pipeline OS & Infrastructure lancé",
...
async def os_reset(current_user: dict = Depends(require_analyste)):
    _clear_status("os")
    return dict(_get_status("os"))
...
async def app_reset(current_user: dict = Depends(require_analyste)):
    _clear_status("app")
    return dict(_get_status("app"))
...
async def apilogs_reset(current_user: dict = Depends(require_analyste)):
