...
class UserService:
    ...
    def _format_user(self, user: dict, include_photo: bool = False) -> dict:
        ...
        if include_photo:
            ...
            result["face_photo"] = decrypt_data(user.get("face_photo", None))
        return result
    ...
    async def signup(self, user_data):
        ...
        user_dict["role"]       = RoleEnum.analyste.value
    ...
    # ── Constantes de lockout (Finding SEC-06 — Audit Bancaire) ──────────────
    # PCI-DSS Req. 8.3.4 : Blocage après N tentatives consécutives
    ...
    async def login(self, email: str, password: str):
        ...
        # Finding SEC-06 (Audit Bancaire) : Vérification du lockout AVANT le check password
        # On vérifie même si l'utilisateur n'existe pas (timing-safe : même réponse)
        if user:
            await self._check_and_enforce_lockout(user, email)

        # Vérification password (timing-constant même si user inexistant)
        password_valid = user is not None and verify_password(password, user["password"])

        if not password_valid:
            # Incrémenter le compteur de tentatives échouées
            if user:
                await self._record_failed_attempt(email, user)
            # Réponse générique intentionnelle (évite l'énumération d'emails)
            raise HTTPException(status_code=401, detail="Email ou mot de passe incorrect")

        if user["role"] == RoleEnum.analyste.value and not user["is_active"]:
            raise HTTPException(
                status_code=403,
    ...
    async def _check_and_enforce_lockout(self, user: dict, email: str):
        """Vérifie si le compte est verrouillé et rejette la tentative si oui."""
        lockout_until = user.get("lockout_until")
        if lockout_until:
            now = datetime.now(timezone.utc)
            # Compatibilité avec datetime naive stocké en MongoDB
            if lockout_until.tzinfo is None:
                from datetime import timezone as tz
                lockout_until = lockout_until.replace(tzinfo=tz.utc)
            if now < lockout_until:
                ...
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        f"En cas de problème, contactez votre administrateur."
                    )
                )
            else:
                # Lockout expiré : nettoyage
                await self.collection.update_one(
                    {"email": email},
                    {"$unset": {"failed_login_attempts": "", "lockout_until": ""}}
                )

    async def _record_failed_attempt(self, email: str, user: dict):
        """Incrémente le compteur d'échecs et applique le lockout si seuil atteint."""
        ...
        if attempts >= self.MAX_FAILED_ATTEMPTS:
            ...
            logging.getLogger(__name__).warning(
                f"🔒 SECURITY: Compte '{email}' verrouillé après {attempts} tentatives échouées. "
                f"Lockout jusqu'à {lockout_until.isoformat()}"
            )

        await self.collection.update_one({"email": email}, update)
    ...
    async def upload_face_photo(self, user_id: str, image_base64: str):
        ...
        # Chiffrement de la photo avant stockage (Bank Grade / RGPD)
        encrypted_photo = encrypt_data(image_base64)

        result = await self.collection.update_one(
            {"_id": ObjectId(user_id)},
            {"$set": {"face_photo": encrypted_photo}}
        )
    ...
    async def update_user(self, user_id: str, data: dict):
        ...
        if "role" in data:
            if data["role"] not in [r.value for r in RoleEnum]:
                raise HTTPException(status_code=422, detail="Rôle invalide (admin ou analyste)")
    ...
    async def forgot_password(self, email: str):
        ...
        code = generate_code()
        # Fix : utiliser datetime timezone-aware (UTC) pour cohérence (Finding PROC-04)
    ...
    async def reset_password(self, email: str, code: str, new_password: str):
        ...
        reset_expiry = user.get("reset_code_expiry")
    ...
    # MFA - REQUEST OTP
    # -------------------------
    async def request_mfa_otp(self, email: str):
        """Génère et envoie un code OTP par email pour le MFA."""
        import hashlib
        user = await self.collection.find_one({"email": email})
        if not user:
            raise HTTPException(status_code=404, detail="Utilisateur non trouvé")
        ...
        code = generate_code()
        expiry = datetime.now(timezone.utc) + timedelta(minutes=15)  # OTP court pour sécu
        
        # AUDIT SEC-04: Hashage du OTP avant stockage (Finding Audit Bancaire)
        # En cas de compromission DB, le code brut n'est pas exposé
        hashed_code = hashlib.sha256(code.encode('utf-8')).hexdigest()
        
        await self.collection.update_one(
            {"email": email},
            {"$set": {"otp_code": hashed_code, "otp_expires_at": expiry}}
        )
        
        # Réutilisation de l'utilitaire d'email (on envoie le code clair par email)
        from email_utils import send_verification_code
        await send_verification_code(email, code)
        return {"message": "Code MFA envoyé par email"}

    # -------------------------
    # MFA - VERIFY OTP
    # -------------------------
    async def verify_mfa_otp(self, email: str, code: str):
        """Vérifie le code OTP pour finaliser la connexion MFA."""
        import hashlib, hmac
        user = await self.collection.find_one({"email": email})
        if not user:
            raise HTTPException(status_code=404, detail="Utilisateur non trouvé")
        ...
        if not stored_hash or not hmac.compare_digest(stored_hash, submitted_hash):
