...
from dependencies import get_current_user, require_analyste
...
async def analyze_pipeline(
    ...
    _user=Depends(require_analyste),
):
...
async def get_status(pipeline: str, _user=Depends(get_current_user)):
...
async def get_results(pipeline: str, _user=Depends(get_current_user)):
    """
    ...
    if not result:
        raise HTTPException(
            status_code=404,
            detail={
                "message": f"Aucun résultat disponible pour le pipeline '{pipeline}'.",
                "reason": "analysis_not_found",
                "action": "Lancez d'abord l'analyse via POST /analyze"
            }
        )
...
async def get_anomaly_detail(
    pipeline: str, anomaly_id: str, _user=Depends(get_current_user)
):
    """
...
async def get_type_report(
    pipeline: str, anomaly_type: str, _user=Depends(get_current_user)
):
    """Retourne le rapport IA complet pour un type d'anomalie spécifique (ou type + risque)."""
    if pipeline not in PIPELINES:
        raise HTTPException(404, "Pipeline inconnu")
    ...
    if anomaly_type in type_analyses:
        return type_analyses[anomaly_type]

    # Recherche insensible à la casse et par préfixe
    for key in type_analyses:
        if key.lower() == anomaly_type.lower():
            return type_analyses[key]
        if key.lower().startswith(anomaly_type.lower() + " + ") or key.lower().startswith(anomaly_type.lower() + " - "):
            return type_analyses[key]
...
async def get_type_correlation(pipeline: str, _user=Depends(get_current_user)):
...
async def reload_kb(_user=Depends(require_analyste)):
...
async def delete_results(pipeline: str, _user=Depends(require_analyste)):
