...
FEATURES = [
    ...
    "running_process_count", "failed_login_count_15min", "config_change_flag",
    "service_restart_count_1h", "uptime_hours",
...
numeric_base = [
    "cpu_usage_percent", "memory_usage_percent", "disk_usage_percent",
    "disk_io_rate", "network_io_rate", "open_file_descriptors",
    "running_process_count", "failed_login_count_15min",
    "service_restart_count_1h", "uptime_hours", "hour", "is_night"
]
num_feats_avail = [f for f in numeric_base if f in df_train.columns]
fig, axes = plt.subplots(3, 4, figsize=(18, 10))
for ax, feat in zip(axes.flatten(), num_feats_avail[:12]):
...
def build_lstm_ae(w, n):
    ...
    m   = Model(inp, out, name="OS_LSTM_AE")
    m.compile(optimizer=Adam(2e-3), loss='mse')
    return m
...
def classify_os_by_rules(row):
    cpu    = row.get("cpu_usage_percent", 0)
    mem    = row.get("memory_usage_percent", 0)
    disk   = row.get("disk_usage_percent", 0)
...
def compute_os_risk(row):
    ...
    mem  = row.get("memory_usage_percent", 0)
    disk = row.get("disk_usage_percent", 0)
    svc  = row.get("service_restart_count_1h", 0)
    log  = row.get("failed_login_count_15min", 0)
    cfg  = row.get("config_change_flag", 0)
    evt_encoded = row.get("event_type", -1)
    try:
        evt_label = label_encoders["event_type"].inverse_transform([int(evt_encoded)])[0]
...
if len(anomaly_idx) > 0:
    ...
    df_result.loc[anomaly_idx, "Risk"] = (
        df_result.loc[anomaly_idx].apply(compute_os_risk, axis=1)
    )
...
for ax, hist, title, c1, c2 in [
    ...
    (axes[1], lstm_hist, "LSTM AE OS",           '#1b4332', 'red')
]:
...
# ================================================================
print("\n" + "=" * 65)
print("EXPORT DES RESULTATS — CSV  →  OS_info/")
print("=" * 65)
...
# ================================================================
print("\n" + "=" * 65)
print("EXPORT MONGODB — Detected_OS_Anomalies.csv")
print("=" * 65)
