# ================================================================
# DETECTION D'ANOMALIES RESEAU — PIPELINE CRISP-DM v5
# ================================================================
...
#   - Detected_Anomalies_anomalies_only.csv
...
import pandas as pd
import matplotlib
matplotlib.use("Agg")          # desactive l'affichage GUI (mode serveur FastAPI)
...
# ── Dossier de sortie dédié au pipeline Firewall ─────────────────
...
print("=" * 65)
print(f"""
Objectif : Detection automatique d'anomalies dans les logs firewall.
...
for ax, feat in zip(axes.flatten(), numeric_feats[:12]):
...
plt.suptitle("Phase 2 — Distributions des features (TRAIN)", fontsize=13, fontweight='bold')
plt.tight_layout()
...
idx_s    = np.random.choice(len(X_train_mm) - LSTM_WINDOW, size=LSTM_SAMPLE, replace=False)
...
lstm_hist = lstm_ae.fit(X_tr_seq, X_tr_seq, epochs=30, batch_size=256,
                        validation_split=0.1,
                        callbacks=[EarlyStopping(patience=5, restore_best_weights=True)],
                        verbose=0)
...
lstm_scores  = (lstm_mse_pad - lstm_mse_pad.min()) / (lstm_mse_pad.max() - lstm_mse_pad.min() + 1e-9)
...
def classify_by_rules(row):
    pc  = row.get("Packet_count", 0);  bs  = row.get("Bytes_sent", 0)
    cc  = row.get("Concurrent_connections", 0)
    lat = row.get("Latency_ms", 0);    pl  = row.get("Packet_loss_pct", 0)
...
if n_ambiguous >= 4:
    ...
    best_k, best_sil = 2, -1
    for k in range(2, min(5, len(X_ambig))):
    ...
    def name_ambig(c):
        if c.get("Packet_count",0) >= 5_000 or c.get("Concurrent_connections",0) >= 5_000: return "DDoS"
        if c.get("Latency_ms",0) >= 300 or c.get("Packet_loss_pct",0) >= 5: return "Latence / Perte paquet"
        if c.get("Packet_count",0) <= 20 and c.get("Bytes_sent",0) <= 5_000: return "Scan reseau"
        if c.get("Dst_port",0) >= 20_000: return "Port inhabituel"
        return "Comportement anormal"
...
for t, c in pd.Series(anomaly_type_array).value_counts().items():
    print(f"       {t:<28}: {c:>5}  ({c/len(anomaly_idx)*100:.1f}%)")


# ── 4.F  SCORE DE RISQUE ─────────────────────────────────────────
def compute_risk(row):
    risk = 4
    if row.get("Packet_count", 0) >= 5_000:           risk += 9
    if row.get("Bytes_sent", 0) >= 1_000_000:         risk += 8
    if row.get("Concurrent_connections", 0) >= 5_000: risk += 7
    if row.get("Latency_ms", 0) >= 300:               risk += 7
    if row.get("Packet_loss_pct", 0) >= 5:            risk += 6
    if row.get("Cpu_usage_fw_pct", 0) >= 85:          risk += 5
    if row.get("Memory_usage_fw_pct", 0) >= 80:       risk += 4
    if row.get("Packet_count", 99) <= 20 and row.get("Bytes_sent", 99) <= 5_000: risk += 5
    if row.get("Dst_port", 0) >= 20_000:              risk += 4
    return min(risk, 10)
...
if len(anomaly_idx) > 0:
    ...
    df_result.loc[anomaly_idx, "Risk"] = df_result.loc[anomaly_idx].apply(compute_risk, axis=1)
...
for ax, hist, title, c1, c2 in [
    (axes[0], ae_hist,   "Autoencoder", 'steelblue', 'orange'),
    (axes[1], lstm_hist, "LSTM AE",     'purple',    'red')]:
...
if n_anom > 5:
    ...
    ax2.set_title(f"PCA 2D — PC1={pca.explained_variance_ratio_[0]*100:.1f}% | PC2={pca.explained_variance_ratio_[1]*100:.1f}%")
...
print("\n  TOP 20 ANOMALIES :")
print(top20[top_cols].to_string(index=False))


# ================================================================
# EXPORT CSV  →  Firewall_info/
# ================================================================
print("\n" + "=" * 65)
print("EXPORT DES RESULTATS — CSV  →  Firewall_info/")
print("=" * 65)
...
# ── Detected_Anomalies.csv  (tous les logs annotes)
df_result[exp_cols].to_csv(OUTPUT_PATH, index=False, encoding="utf-8-sig")

# ── Detected_Anomalies_anomalies_only.csv  (anomalies uniquement)
...
# ================================================================
# EXPORT MONGODB — Detected_Anomalies.csv uniquement
# ================================================================
print("\n" + "=" * 65)
print("EXPORT MONGODB — Detected_Anomalies.csv")
print("=" * 65)

def _to_native(v):
    """Convertit les types numpy en types Python natifs (requis par BSON)."""
...
def save_detected_anomalies_to_mongo(df: pd.DataFrame, run_ts: str) -> None:
    """
    Enregistre dans MongoDB le contenu exact de Detected_Anomalies.csv.

    - Upsert par Event_id si la colonne existe  (pas de doublons entre runs)
    - Insert simple sinon                        (remplace les docs du meme run)
    - Champ 'pipeline_run_at' ajoute a chaque document (tracabilite du run)
    - Non bloquant : une erreur MongoDB n'arrete pas le pipeline
    """
    ...
    try:
        ...
        client.server_info()                      # leve une exception si non joignable
        col = client[MONGO_DB][MONGO_COL]

        # Preparation : NaN/Inf -> None, types numpy -> python natifs
        df_clean = df.copy()
        df_clean.replace([float("inf"), float("-inf")], None, inplace=True)
        df_clean = df_clean.where(df_clean.notna(), other=None)
        df_clean["pipeline_run_at"] = run_ts      # metadata de tracabilite
        ...
        if has_event_id:
            # Upsert : met a jour si Event_id existe, insere sinon
            operations = [
            ...
            # Pas d'identifiant metier : supprime l'ancien run et reinsere
        ...
        # Index (idempotents — crees une seule fois)
...
# Horodatage UTC unique pour identifier ce run dans MongoDB
run_timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

# !! Detected_Anomalies.csv uniquement — PAS _anomalies_only.csv !!
save_detected_anomalies_to_mongo(df_result[exp_cols], run_timestamp)
